...
def start_information(args, colors):
    ...
    print(display_info("Informações do Sistema", colors))

    sys_info = f"Usuário: {current_user}\n" \
               f"Máquina: {hostname}\n" \
    ...
    if args.directory:
        sys_info += f"Diretórios: {', '.join(os.path.abspath(directory) for directory in args.directory)}"

    if args.file:
        sys_info += f"Arquivos: {', '.join(os.path.abspath(file_path) for file_path in args.file)}"
    ...
    if len(supported_files) != 0:
        for value in supported_files.values():
            for file_path in value:
                print(f"Arquivo: {os.path.abspath(file_path)} ({convert_size(os.path.getsize(file_path))}) MIME: {guess_mime(file_path) or from_file(file_path, mime=True)}")
...
def process_directory_argument(args):
    ...
    for argument in args.directory:
        try:
            ...
            print(f"Permissões insuficientes para ler: {error.filename}")

    if len(supported_files) != 0:
        print(f"Os diretórios possuem  {sum(len(value) for value in supported_files.values())} Itens Suportados. {convert_size(supported_files_size)}")
...
def process_contents(args, colors):
    print(display_info(f"Enumerando conteúdos [{time.strftime('%H:%M:%S %d/%b/%Y')}]", colors))
    if args.directory:

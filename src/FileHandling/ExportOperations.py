...
def get_timestamp_filename(dst_path, extension):
    ...
    return os.path.abspath(f"{dst_path}/SearchParty-Results-{timestamp}{extension}")
...
def export_to_csv(args, dst_path, data_found):
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        csv_writer.writerow(['data_type', 'files'])
...
def export_to_database(args, dst_path, data_found):
    ...
    for table_name, file_list in data_found.items():
        if file_list:
            cursor.execute(f'''CREATE TABLE IF NOT EXISTS "{table_name}" (files TEXT)''')
            for info in file_list:
                if args.findme:

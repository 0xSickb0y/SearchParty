...
def validate_file_operations(args, colors):
    if args.copy and (args.move or args.delete):
        raise ValueError(display_alert("Error: -copy e (-move, -delete) não podem ser fornecidos simultaneamente", colors=colors))
    if args.move and (args.copy or args.delete):
        raise ValueError(display_alert("Error: -move e (-copy, -delete) não podem ser fornecidos simultaneamente", colors=colors))
    if args.delete and (args.copy or args.move):
        raise ValueError(display_alert("Error: -delete e (-copy, -move) não podem ser fornecidos simultaneamente", colors=colors))
    if args.delete and args.save:
        raise ValueError(display_alert("Error: -delete e -save não podem ser fornecidos simultaneamente", colors=colors))
    if args.move and args.save:
        raise ValueError(display_alert("Error: -move e -save não podem ser fornecidos simultaneamente", colors=colors))


def validate_findme_and_datatype_args(args, colors):
    if args.findme and args.data_filters:
        raise ValueError(display_alert("Error: As opções -find e -datatype options não podem ser fornecidos simultaneamente", colors=colors))
    ...
    if args.data_filters:
        ...
        for dt in args.data_filters:
            if dt not in data_filters:
                raise ValueError(display_alert(f"Error: Tipos de dados inválidos: {dt}", colors=colors))
        filtered_types = []
    ...
    if args.file_filters:
        ...
        for ft in args.file_filters:
            if ft not in file_types:
                raise ValueError(display_alert(f"Error: Tipos de arquivos inválidos: {ft}", colors=colors))
            if ft in ['bmp', 'png', 'gif', 'tiff', 'jpeg', 'webp'] and not args.ocr_enabled:
                raise ValueError(display_alert(f"Error: Tipo {ft} não pode ser usado sem capacidades OCR", colors=colors))
        filtered_types = []
...
def validate_save(args, colors):
    ...
    if args.save:
        ...
        if not writable_path:
            raise PermissionError(display_alert(f"Permissões insuficientes para salvar resultados em: {paths_to_check}", colors=colors))

        for st in save_types:
            if st not in valid_save_types:
                raise ValueError(display_alert(f"Error: Opção de exportação inválida {st}. Deve ser um dos seguintes: {', '.join(valid_save_types)}", colors=colors))
    ...
    if args.copy:
        ...
        if os.path.isfile(args.copy):
            ...
            if os.access(current, os.W_OK):
                ...
                raise PermissionError(display_alert(f"Error: Permissões insuficientes: {current}", colors=colors))
            ...
            if os.path.exists(default):
                ...
                raise PermissionError(display_alert(f"Error: Permissões insuficientes: {args.copy}", colors=colors))

            elif os.path.exists(args.copy) and not os.access(parent_dir, os.W_OK):
                raise PermissionError(display_alert(f"Error: Permissões insuficientes: {parent_dir}", colors=colors))
    ...
    if args.move:
        ...
        if os.path.isfile(args.move):
            ...
            if os.path.exists(default):
                ...
                raise PermissionError(display_alert(f"Error: Permissões insuficientes: {args.move}", colors=colors))
